...
const contactUserClient = createApiClient({
    baseURL: import.meta.env.VITE_API_BASE_URL_STUDENT || "http://localhost:3031/student",
    useAuth: false,
});
...
export const contactUserService = {
    // Signup endpoint
    signup: async (data: SignupPayload): Promise<{ data: any } | { error: ApiError }> => {
        try {
    ...
    // Login endpoint
    login: async (data: LoginPayload): Promise<{ data: any } | { error: ApiError }> => {
        try {
    ...
    // Update endpoint
    update: async (userId: string, data: UpdatePayload): Promise<{ data: any } | { error: ApiError }> => {
        try {
            const filteredPayload = filterPayload(data);
            const response = await contactUserApi.put(
                `/${userId}`,
                filteredPayload,
                {
                    headers: {
